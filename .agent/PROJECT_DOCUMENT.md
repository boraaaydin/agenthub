...
starting at `1`, include a lifecycle status (`open`, `task_creating`, `task_created`, `in_progress`, `completed`, or `cancelled`), a `kind` (`workitem` or `draft`), an optional completion timestamp, and same-project `dependencyIds`. Every new workitem is created as a draft with open status; drafts cannot be dependencies or enter planning flows, and are promoted to workitems, which opens an accessible prompt to create a task when applications and dependencies allow it. A dependency is finished when it is `completed` or `cancelled`; otherwise it blocks the dependent workitem from starting a planning session. Workitems are listed in a semantic table with
