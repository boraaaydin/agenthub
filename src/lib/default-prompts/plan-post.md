...
1. Read the plan file created or updated during this planning flow from `.agent/plans/`.
2. Confirm the plan file matches the task's language (or an explicitly requested language), has a focused scope, names the root `{{PROJECT_SLUG}}` application, and includes Description, Dependencies, Context, measurable Acceptance Criteria, Technical Notes, and Verification.
3. Check that dependencies refer to plan files in the project's plan queue or archive, never to GitHub issues.
...
5. Correct only omissions or ambiguities in the plan file. Do not modify source code, runtime data, or unrelated plan files.

Use AgentHub's local task workflow only:
- Active plans: `.agent/plans/`
- Completed-plan decision log: `.agent/plans-archived/YYYY/MM/DD/`

Do not create, reference, comment on, or otherwise interact with GitHub issues.

6. End by reporting the final plan path, dependencies, and any assumptions recorded in the plan. Do not end the CLI process yet: first complete the registration instruction that appears later in this composed prompt.
