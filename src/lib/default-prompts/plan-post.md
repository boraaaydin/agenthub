...
2. Confirm the task file matches the task's language (or an explicitly requested language), has a focused scope, names the root `agenthub` application, and includes Description, Dependencies, Context, measurable Acceptance Criteria, Technical Notes, and Verification.
