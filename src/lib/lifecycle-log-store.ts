...
export async function appendLifecycleEvent(input: LifecycleEventInput): Promise<LifecycleEvent> { return serializeWrite(async () => { const document = await readDocument(); const event = { id: randomUUID(), ...input } as LifecycleEvent; document.events.push(event); await writeDocument(document); return event; }); }
