...
export async function PATCH(request: Request, context: RouteContext<"/api/tasks/[taskId]">) {
  ...
  try {
    ...
    if (patch.applicationId !== undefined || patch.projectId !== undefined) {
      const applicationId = patch.applicationId ?? task.applicationId;
    ...
    if (!await getWorkitem(project.id, workitemId)) {
...
export async function DELETE(request: Request, context: RouteContext<"/api/tasks/[taskId]">) {
  ...
  try {
    ...
    return deleted
      ? Response.json({ ...deleted, fileDeleted, ...(fileError ? { fileError } : {}) })
      : missing();
  } catch (error) {
