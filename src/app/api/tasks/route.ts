...
export async function GET(request: Request) {
  try {
    ...
    const status = taskFilterStatus(
      ...
      url.searchParams.get("all") ?? undefined,
    );
    ...
    return Response.json(
      { error: "Task data could not be read. Check data/tasks.json and try again." },
      { status: 500 },
    );
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { projectId, applicationId, workitemId } = input as Record<string, unknown>;
    ...
    const application = await getApplication(applicationId.trim());
    if (!application) {
      return Response.json({ error: "Application not found." }, { status: 404 });
    }
    if (application.projectId !== project.id) {
      return Response.json(
        { error: "The selected application does not belong to this task's project." },
        { status: 400 },
      );
    }
    ...
    const normalizedWorkitemId = typeof workitemId === "number"
      ? workitemId
      : typeof workitemId === "string" && /^\d+$/.test(workitemId)
        ? Number(workitemId)
        : Number.NaN;
