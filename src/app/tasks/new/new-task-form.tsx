...
export function NewTaskForm({ projects, tasksByProject, initialProjectId, error: loadError }: NewTaskFormProps) {
    ...
    <main className="min-h-screen bg-[#f4f6fa] px-4 py-6 text-slate-900 sm:px-6 sm:py-10">
      <div className="mx-auto w-full max-w-2xl">
        <header className="border-b border-slate-200 pb-5">
          ...
          <p className="mt-1 text-sm leading-6 text-slate-600">Register a task file that was created outside a taskning session.</p>
        </header>
        {loadError ? <p role="alert" className="mt-8 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">{loadError}</p> : projects.length === 0 ? (
          ...
          <form onSubmit={createTask} className="mt-8 space-y-6" noValidate>
            <div className="grid gap-6 sm:grid-cols-2">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-800" htmlFor="new-task-task">Task</label>
                <select id="new-task-task" value={taskId} onChange={(event) => setTaskId(event.target.value)} disabled={isSubmitting || !projectId} className="mt-2 h-11 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm outline-none transition focus:border-sky-600 focus:ring-3 focus:ring-sky-100 disabled:cursor-not-allowed disabled:bg-slate-100">
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2">
              <button type="submit" disabled={isSubmitting} className="h-11 rounded-xl bg-sky-700 px-5 text-sm font-semibold text-white shadow-sm transition hover:bg-sky-800 focus:outline-none focus:ring-3 focus:ring-sky-200 disabled:cursor-not-allowed disabled:bg-slate-300">{isSubmitting ? "Creating task…" : "Create task"}</button>
