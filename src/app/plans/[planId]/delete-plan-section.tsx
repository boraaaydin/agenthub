...
export function DeletePlanSection({ apiPath, filePath, disabled, onError, onDeleted }: DeletePlanSectionProps) {
  ...
  async function deletePlan() {
    ...
    try {
      ...
      if (!response.ok) {
        onError(body.error ?? "Unable to delete the plan. Try again.");
        return;
      }
  ...
  return (
    <section className="mt-10 border-t border-slate-200 pt-6" aria-labelledby="delete-plan">
      <h2 id="delete-plan" className="text-sm font-semibold text-slate-900">Delete plan</h2>
      <p className="mt-1 text-sm leading-6 text-slate-600">This permanently removes the plan record. Its file stays on disk unless you explicitly choose to remove it too.</p>
      {isConfirming ? (
        <div className="mt-4 space-y-4 rounded-xl border border-red-200 bg-red-50 p-4">
          <label className="flex cursor-pointer items-start gap-3 text-sm leading-6 text-slate-800">
            ...
            />
            <span>Also delete the plan file from disk (<span className="break-all font-mono text-xs">{filePath}</span>)</span>
          </label>
          <div className="flex flex-wrap items-center gap-3">
            <button type="button" onClick={deletePlan} disabled={disabled || isDeleting} className="h-11 rounded-xl bg-red-700 px-5 text-sm font-semibold text-white shadow-sm transition hover:bg-red-800 focus:outline-none focus:ring-3 focus:ring-red-200 disabled:cursor-not-allowed disabled:bg-red-300">
              {isDeleting ? "Deleting plan…" : "Confirm delete"}
            </button>
