...
export function PlanFilePreview({ filePath, projectPath, result }: PlanFilePreviewProps) {
  const repositoryPath = projectPath ? `${projectPath}/${filePath}` : filePath;

  return (
    <section className="mt-10 rounded-xl border border-slate-200 bg-white p-5 shadow-sm" aria-labelledby="plan-file">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <div>
          <h2 id="plan-file" className="text-lg font-semibold text-slate-900">Plan file</h2>
      ...
      {result.status === "ok" ? (
        ...
        <p className="mt-4 rounded-lg bg-slate-50 px-4 py-3 text-sm leading-6 text-slate-700">
          {result.status === "not-found" && <>The plan file was not found at <span className="font-mono text-xs">{repositoryPath}</span>.</>}
          {result.status === "too-large" && "This plan file is too large to preview (maximum 512 KB)."}
          {result.status === "invalid-path" && "This path is outside the project directory and cannot be previewed."}
          {result.status === "missing-project" && "The project record is missing, so this plan file cannot be located."}
